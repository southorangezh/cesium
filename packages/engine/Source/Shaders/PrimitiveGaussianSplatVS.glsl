...
void main() {
    ...
    uvec4 posData = uvec4(texelFetch(u_splatAttributeTexture, posCoord, 0));
    uint plyIndex = posData.w;  // PLY 索引存储在 texel 0 的第 4 个分量

    // ========================================
    // [方案1] 使用已读取的位置数据
    // ========================================
    vec4 splatPosition = vec4(uintBitsToFloat(posData.xyz), 0.0);
